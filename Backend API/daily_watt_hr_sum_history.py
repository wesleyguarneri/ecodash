...
logging.info('client print statement: ' );
...
#dynamic dictrionary which will store the eigmtere name and their corresponding daily wathtotal
#when first initiated it only holds building ids as values but only bc I was lazy and copied above dictrionary 
wath_reported_eigs = {"eigmeter-alpha-gamma-delta-bldg95_dept_lehigh_edu":"0095" ,
    "eigmeter-alphachiomega-bldg93_res_lehigh_edu":"0093" ,
    "eigmeter-alphaphi-bldg98_res_lehigh_edu":"0098" ,
    "eigmeter-alphatauomega-bldg99_res_lehigh_edu":"0099" ,
    "eigmeter-alumni-bldg27_dept_lehigh_edu":"0027" ,
...
# watt = sqrt(3)*((sqrt(3)*((last_insvan+last_insvbn+last_insvcn)/3))*((last_insia+last_insib+last_insic)/3)*((last_inspf_a+last_inspf_b+last_inspf_c)/3)))
# watt Hour = w * 1 / (num periods per hour)
# kilowatt hours = watt hour /1000 (edited) 
...
tot_df = pd.DataFrame()
for i in range(365):
    print("From : " + str(interval_1) + " to : " + str(interval_2))
    #query data 
    query_one_day = 'SELECT host, time, wathtot FROM eig WHERE time < now() - '+str(interval_1)+'m and time > now() - '+str(interval_2)+'m'
    query_two_day = 'SELECT host, time, wathtot FROM eig WHERE time < now() - '+str(interval_2)+'m and time > now() - '+str(interval_3)+'m '
    reg_query_first=client.query(query_one_day)
    reg_query_second=client.query(query_two_day)
    interval_1+=1440
    interval_2+=1440
    interval_3+=1440
    #convert data to usable format
    cpu_points_1 = list(reg_query_first.get_points(measurement='eig'))
    cpu_points_2 = list(reg_query_second.get_points(measurement='eig'))
    # print(cpu_points_1[4]["host"])
    # print("Number of eigmeters recorded from ",interval_1," mins --> ", interval_2, "mins : ",len(cpu_points_1))
    # print("Number of eigmeters recorded from ",interval_2," mins --> ", interval_3, "mins : ",len(cpu_points_2))
    # print(len(cpu_points_2))

    # generate lists of eigmeters for each day (so that missing ones can be determined)
    # populate the wath_reported_eigs dictionary 
    cpu_hosts_1 = []
    cpu_hosts_2 = []
    for i in range(len(cpu_points_1)):
        cpu_hosts_1.append(cpu_points_1[i]["host"])
        wath_reported_eigs[cpu_points_1[i]["host"]]= cpu_points_1[i]["wathtot"]
    for i in range(len(cpu_points_2)):
        cpu_hosts_2.append(cpu_points_2[i]["host"])
        wath_reported_eigs[cpu_points_2[i]["host"]]-=  cpu_points_2[i]["wathtot"]

    # for key, value in wath_reported_eigs.items():
    #     print(key, '->', value)

    #Get the eigmeters which are not reported on in exactly both days so their buildingID's are not incorporated into the table append 
    list_difference_1 = [item for item in cpu_hosts_1 if item not in cpu_hosts_2]
    list_difference_2 = [item for item in cpu_hosts_2 if item not in cpu_hosts_1]
    # print("list_difference: ",list_difference_1, list_difference_2) 

    # appending [ BUILDINGID, WATHTODAY, TIME]
    building_metadata_incr = []
    for key,value in wath_reported_eigs.items():
        building_metadata_incr.append([eigs_build_dict[key],value, cpu_points_1[0]["time"]])

    incr_df = pd.DataFrame(building_metadata_incr, columns = ['BUILDINGID', 'wathtot', 'time'])
    incr_df = incr_df.groupby('BUILDINGID', as_index=False).agg(wathtot = ('wathtot', 'sum'))
    #group by gets rid of the time column so it needs to be placed back into the df 
    incr_df['time'] = cpu_points_1[0]["time"]

    #filter out the BIDs with missing eigmeters
    for i in list_difference_1:
        incr_df = incr_df.drop(incr_df[incr_df.BUILDINGID == eigs_build_dict[i]].index)
    for i in list_difference_2:
        incr_df = incr_df.drop(incr_df[incr_df.BUILDINGID == eigs_build_dict[i]].index)

    # print(temp_df)
    tot_df = tot_df.append(incr_df, ignore_index=True)

print("Outside the for loop finally ")
...
# Write data to the temp csv using pandas dataframe
tot_df.to_csv("AnnualWattHrSum.csv")

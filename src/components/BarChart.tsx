...
const BarChart: React.FC<BarChartProps> = ({ bId, timeFrame }) => {
  ...
  const createQuery = (): Query => {
    ...
    q.where = `BUILDINGID = '${bId}' AND time >= CURRENT_TIMESTAMP - INTERVAL '7' DAY AND time <= CURRENT_TIMESTAMP - INTERVAL '1' DAY`;
    ...
    const opts: Highcharts.Options = {
      ...
      tooltip: {
        pointFormatter: function () {
          ...
          //   minute: "2-digit",
          // });
          const energy = parseInt(this.y + "").toLocaleString() + " kWh";

          return `Used ${energy}`;
        },
      },
      title: {
        text: "Wattage over the Previous Week",
      },
      yAxis: {
        title: {
          text: "Wattage (kW)",
